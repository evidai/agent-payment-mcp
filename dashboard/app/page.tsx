...
// v2 MCP snippet — replaces the deprecated KYAPay cURL example.
// Shows the user how to drop a service into claude_desktop_config.json
// with their LEMON_CAKE_PERMIT (no JWT minting required).
function CurlSnippet({ service }: {
  service: Service;
  // The old props (tokenType / apiKey / buyerTag / expiry) are gone in v2.
  // Kept the function name + ServiceDetail call site to minimise churn.
  tokenType?: string; apiKey?: string; buyerTag?: string; expiry?: string;
}) {
  const t = useT();
  const [copied, setCopied] = useState(false);
  const config = JSON.stringify({
    mcpServers: {
      [service.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 24)]: {
        command: "npx",
        args: ["-y", "pay-per-call-mcp"],
        env: {
          LEMON_CAKE_PERMIT: "<permit from /start/v2>",
          LEMON_CAKE_SERVICE_ID: service.id,
        },
      },
    },
  }, null, 2);
  ...
  return (
    <div className="relative bg-gray-950 rounded-xl overflow-hidden">
      ...
      <pre className="p-4 text-xs font-mono text-gray-300 overflow-x-auto leading-relaxed">{config}</pre>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  // Legacy buyer API key state from v1 — kept only so localStorage values
  // don't crash older browsers. The page no longer renders it. Remove after
  // ~30 days when most clients have rolled to the new key (~ 2026-06-22).
  const [apiKeys,        setApiKeys]        = useState<Array<{ id: string; key: string; label: string; createdAt: string; revoked: boolean }>>([]);
