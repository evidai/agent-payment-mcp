...
// ── Types ─────────────────────────────────────────────────────────────────────
// v2 admin nav. Custody-era sections (buyers, jpyc) are gone; v2 introduces
// providers / subscriptions / charges / invoices / offramp. Legacy sections
// (marketplace, monitoring, finance) survive but get scoped to v2 data
// where they overlap.
type NavSection =
  | "overview"
  | "providers"
...
// ── Admin Sidebar ─────────────────────────────────────────────────────────────
const NAV: {id:NavSection; label:string; sub:string; Icon: ({cls}:{cls?:string})=>React.JSX.Element; group?: "v2" | "legacy"}[] = [
  // ── v2 primary（permit + サブスク） ─────────────────────────
  { id:"overview",      label:"概要",            sub:"MRR / 流通量 / 主要 KPI", Icon: Ico.Overview,    group:"v2" },
  { id:"providers",     label:"Provider",        sub:"v2 提供者一覧 + プラン",  Icon: Ico.User,        group:"v2" },
  { id:"subscriptions", label:"サブスク",        sub:"Stripe 連動・プラン分布", Icon: Ico.Finance,     group:"v2" },
  { id:"activity",      label:"アクティビティ",  sub:"PermitCharge ライブ",     Icon: Ico.Bolt,        group:"v2" },
  { id:"invoices",      label:"インボイス",      sub:"適格請求書 (T+13)",       Icon: Ico.Finance,     group:"v2" },
  { id:"offramp",       label:"オフランプ",      sub:"USDC → JPY 出金履歴",      Icon: Ico.Bolt,        group:"v2" },
  // ── legacy（v1 残存・段階的に削除） ─────────────────────────
  { id:"marketplace",   label:"マーケットプレイス", sub:"サービス審査",          Icon: Ico.Marketplace, group:"legacy" },
  { id:"monitoring",    label:"監視",            sub:"v1 ログ・リスク",         Icon: Ico.Monitor,     group:"legacy" },
  { id:"finance",       label:"財務（v1）",      sub:"旧 payout / fee 設定",     Icon: Ico.Finance,     group:"legacy" },
];
  ...
  return (
    <aside className="w-[260px] sm:w-60 h-full flex flex-col bg-white border-r border-gray-200 flex-shrink-0">
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto min-h-0">
        {/* v2 primary */}
        <p className="px-3 mb-2 text-[10px] font-bold uppercase tracking-wider text-gray-400">v2</p>
        <div className="space-y-1">
          {NAV.filter(n => n.group === "v2").map(item => {
        ...
        {/* legacy */}
        <p className="px-3 mt-6 mb-2 text-[10px] font-bold uppercase tracking-wider text-gray-400">Legacy (v1)</p>
        <div className="space-y-1">
          {NAV.filter(n => n.group === "legacy").map(item => {
            const badge = item.id==="marketplace"?pendingSvc : item.id==="monitoring"?openFlags : 0;
            const active = nav === item.id;
            return (
              <button key={item.id} onClick={()=>setNav(item.id)}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-left transition-all group opacity-70 hover:opacity-100 ${active?"bg-gray-100 text-gray-900 opacity-100":"text-gray-500 hover:bg-gray-50 hover:text-gray-800"}`}>
                <item.Icon cls={`w-4 h-4 flex-shrink-0 ${active?"text-gray-700":"text-gray-400"}`}/>
                <div className="flex-1 min-w-0">
                  <p className={`text-[12px] font-medium leading-tight ${active?"text-gray-900":"text-gray-500"}`}>{item.label}</p>
                </div>
                {badge > 0 && (
                  <span className={`flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold ${item.id==="monitoring"&&openFlags>0?"bg-red-500 text-white":"bg-amber-400 text-gray-900"}`}>{badge}</span>
                )}
              </button>
            );
          })}
...
const PAGE_META: Record<NavSection, {title:string; desc:string}> = {
  ...
  marketplace:   { title:"マーケットプレイス運営", desc:"サービス審査・Verified付与・アカウント管理 (v1)" },
  monitoring:    { title:"監視（v1）",           desc:"v1 ログ・リスクフラグ・サーキットブレーカー" },
  finance:       { title:"財務・設定（v1）",      desc:"旧 payout / fee / システムパラメーター" },
};
